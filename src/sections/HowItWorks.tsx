...
const baseLineData = [60, 65, 62, 70, 68, 75, 80];
const baseBarData = [2, 2.5, 2.2, 3, 2.8, 3.5, 4, 3.8, 4.2, 4.5, 4.8, 5, 5.2, 5.5, 5.8, 6, 6.2, 6.5, 6.8, 7];
...
const HowItWorks = () => {
  // Animation state for line and bar
  const [lineData, setLineData] = useState({
    labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'],
    datasets: [
      {
        label: 'Customer Experience Index',
        data: Array(baseLineData.length).fill(0),
        borderColor: '#000',
    ...
    datasets: [
      {
        ...
        data: Array(baseBarData.length).fill(0),
        backgroundColor: baseBarData.map(() => 'rgba(0,0,0,0.15)'),
        borderRadius: 4,
        barPercentage: 0.7,
    ...
    const animateLine = () => {
      ...
      setLineData((prev) => ({
        ...
        datasets: [
          {
            ...prev.datasets[0],
            data: baseLineData.map((val) => val * (lineStep / lineSteps)),
          },
        ],
      }));
      if (lineStep < lineSteps) {
    ...
    const animateBar = () => {
      ...
      setBarData((prev) => ({
        ...
        datasets: [
          {
            ...prev.datasets[0],
            data: baseBarData.map((val) => val * (barStep / barSteps)),
            backgroundColor: baseBarData.map(() =>
              `rgba(0,0,0,${0.15 + 0.85 * (barStep / barSteps)})`
            ),
          },
        ],
      }));
  ...
  const lineOptions = {
    ...
    animation: false as const, // Fix linter error
  };
